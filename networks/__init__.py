...
def get_networks(state, N=None, arch=None):
  ...
  if state.input_size not in cls.supported_dims:
    raise RuntimeError("{} doesn't support input size {}".format(
        cls.__name__, state.input_size))
  logging.info('Build {} {} network(s) with [{}({})] init'.format(
      N, arch, state.init, state.init_param))
  nets = []
  for n in range(N):
    net = cls(state).to(state.device)
