...
class Trainer(object):
  ...
  def init_data_optim(self):
    ...
    # logging.info('Created distill label is %s', distill_label)
    ...
    # logging.info('Created distill data is %s', distill_data)
    ...
    # logging.info('Created learning rate is %s', self.raw_distill_lrs)
    ...
    self.scheduler = optim.lr_scheduler.StepLR(
        self.optimizer, step_size=state.decay_epochs, gamma=state.decay_factor)
    for p in self.params:
      p.grad = torch.zeros_like(p)

  def get_steps(self):
    data_label_iterable = (
        x for _ in range(self.state.distill_epochs)
    ...
    # logging.info('Example Data %s, label %s, lr %s', steps[0][0], steps[0][1],
    #              steps[0][2])
    return steps
  ...
  def backward(self, model, rdata, rlabel, steps, saved_for_backward):
    ...
    for (data, label, lr), w, gw in reversed(list(zip(steps, params, gws))):
      ...
      hvp_grad = torch.autograd.grad(
          outputs=(gw,), inputs=hvp_in, grad_outputs=(dgw,))
    ...
    # logging.info('Example gdata %s', gdatas[0])
    # logging.info('Example glr %s with lr %s', glrs[0], lrs[0])
  ...
  def train(self):
    ...
    for epoch, it, (rdata, rlabel) in self.prefetch_train_loader_iter():
      ...
      if it == 0 and ((ckpt_int >= 0 and epoch % ckpt_int == 0) or epoch == 0):
        ...
        self.save_results(
            steps=steps, subfolder='checkpoints/epoch{:04d}'.format(epoch))
        evaluate_steps(state, steps, 'Begin of epoch {}'.format(epoch))

      do_log_this_iter = it == 0 or (state.log_interval >= 0 and
                                     it % state.log_interval == 0)

      self.optimizer.zero_grad()
      rdata, rlabel = rdata.to(
          device, non_blocking=True), rlabel.to(
      ...
      if sample_n_nets == state.local_n_nets:
        ...
        idxs = np.random.choice(
            state.local_n_nets, sample_n_nets, replace=False)
      ...
      # logging.info('accumulated finished.')
      # logging.info('Check lr %s', self.raw_distill_lrs)
      # logging.info('Check data %s', self.data[0])
      ...
      if do_log_this_iter:
        loss = losses.item()
        logging.info(('Epoch: {:4d} [{:7d}/{:7d} ({:2.0f}%)]\tLoss: {:.4f}\t'
                      'Data Time: {:.2f}s\tTrain Time: {:.2f}s').format(
                          epoch,
                          it * train_loader.batch_size,
                          len(train_loader.dataset),
                          100. * it / len(train_loader),
                          loss,
                          data_t,
                          t,
                      ))
